...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            CASE 
                WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid, 4, LEN(cid)) -- Rempve 'NAS' prefix if present
                ELSE cid
            END AS cid,
            CASE
                WHEN bdate > GETDATE() THEN NULL
                ELSE bdate
            END AS bdate, -- Set future birtdates to NULL
            CASE
                WHEN UPPER(TRIM(gen)) IN ('F', 'FEMALE') THEN 'Female'
                WHEN UPPER(TRIM(gen)) IN ('M', 'MALE') THEN 'Male'
                ELSE 'n/a'
            END AS gen
        -- Normalize gender values and handle unknwn cases
        ...
        SELECT
            ...
            CASE WHEN TRIM(cntry) = 'DE' THEN 'Germany'
                ...
                ELSE TRIM(cntry)
            END AS cntry
        -- Normaliza and Handle missing or blanck country codes
